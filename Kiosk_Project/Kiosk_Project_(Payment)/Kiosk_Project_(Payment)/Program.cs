...
namespace BackLogTasks{    class AssembleFunctions    {
        ...
        static string UserInputOfItemsAndValidation(bool plural)        {            string itemInput = "";            bool valid = false;            while (valid == false)            {                if (plural == false)                {
                    ...
                    Console.WriteLine("Input the cost of an another item. (Or press ENTER without typing to proceed to pay.)");                }                itemInput = Console.ReadLine();                char[] arr = new char[itemInput.Length];                itemInput.CopyTo(0, arr, 0, itemInput.Length);                bool decimalUsed = false;                bool errorFound = false;                for (int i = 0; i < arr.Length; i++)                {                    if (errorFound == false)                    {                        if (arr[i] != '1' && arr[i] != '2' && arr[i] != '3' && arr[i] != '4' && arr[i] != '5' && arr[i] != '6' && arr[i] != '7' && arr[i] != '8' &&
                            ...
                            else                            {                                Console.WriteLine("Whoops! We need you to enter a non-negative number. Try again.");                                errorFound = true;                                decimalUsed = false;                            }//It's not a valid character.
                        }//It is not a number.
        ...
        static void Main(string[] args)
        {
            ...
            string generic = report[0] + " " + report[1] + " " + report[2] + " " + report[3];            ProcessStartInfo startInfo = new ProcessStartInfo();            string userName = Environment.UserName;            startInfo.FileName = "C:\\Users\\" + userName + "\\source\\repos\\Kiosk_Project_(Log_Recorder)\\Kiosk_Project_(Log_Recorder)\\bin\\Debug\\net6.0\\Kiosk_Project_(Log_Recorder).exe";
